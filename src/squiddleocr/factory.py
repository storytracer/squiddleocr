...
def build_pipeline(model: str | Path = DEFAULT_SIZE, *, pipeline: str = "paddle", layout: str = "paddle",
    ...
    ``kraken`` command does.
    ``eynollah`` (also ``layout="eynollah"``): eynollah's regions, reading order and line polygons
    from its PAGE-XML (eynollah runs as a subprocess, see ``squiddleocr.eynollah``), lines read by
    kraken along a synthesised baseline; no tables or formulas stage. ``eynollah`` carries the run
    ...
    if pipeline == "eynollah" or layout == "eynollah":
        ...
        return Pipeline(recognizer=recognizer, detector=det, layout=lay, detect_per_region=True, keep_line_order=True)
